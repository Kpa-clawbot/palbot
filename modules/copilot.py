    ...
    async def _collect_recent_images(self, ctx, lookback: int) -> list:
        ...
        async def _add_from_message(msg):
            ...
            for embed in msg.embeds:
                self.bot.logger.debug(f"[img-embed] type={embed.type}, url={getattr(embed, 'url', None)}, has_image={embed.image is not None}, has_thumb={embed.thumbnail is not None}")
            ...
            if msg.content:
                ...
                for url_match in self.IMAGE_URL_PATTERN.finditer(msg.content):
                    ...
                    if raw_url in embed_urls:
                        self.bot.logger.debug(f"[img-probe] skipped (embed dup): {raw_url[:80]}")
                        continue
                    url_count += 1
                    try:
                        self.bot.logger.debug(f"[img-probe] trying: {raw_url[:120]}")
                        img_bytes, mime = await self._download_url(raw_url, probe=True)
                        if img_bytes and mime:
                            self.bot.logger.debug(f"[img-probe] got image: {mime}, {len(img_bytes)} bytes")
                            img_bytes, mime = self._process_image_bytes(img_bytes, mime)
                            b64 = base64.b64encode(img_bytes).decode("ascii")
                            images.append({
                                "url": f"data:{mime};base64,{b64}",
                            ...
                            self.bot.logger.debug(f"[img-probe] not an image: {raw_url[:80]}")
                    except Exception as e:
                        self.bot.logger.debug(f"[img-probe] error: {e} for {raw_url[:80]}")
                        continue
