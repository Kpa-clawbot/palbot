...
Keep the summary under {compact_max_tokens} tokens."""
    ...
    async def _build_compacted_context(self, ctx, settings: dict, token, base_url) -> str:
        ...
        if cache is not None:
            ...
            # Only count tokens BEYOND the configured raw_hours window for the
            # token trigger — the raw window always stretches back to the compaction
            # boundary, so in a busy channel it can be large even right after compaction.
